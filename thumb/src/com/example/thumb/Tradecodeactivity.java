...
public class Tradecodeactivity extends Activity implements View.OnClickListener{
		...
		public void handleMessage(android.os.Message msg) { 
			...
			try {
				...
				if(statecode.equals("800")){
					...
					editor.commit();
					Toast.makeText(getApplicationContext(), "设置成功", 1000).show();
					finish();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(sp.getBoolean("issettrade", false)){
			tv1.setText("修改交易密码");
			tv2.setText("");
		}else{
			tv1.setText("设置交易密码");
			tv2.setText("为了您的交易安全，请设置您的交易密码");
			tv4.setText("");
		}
		...
		case R.id.button15: 
			if(array[5].equals("a")){
				...
				if(btn15.getText().toString().equals("下一步")){
					...
					code1();
					btn15.setText("完成");
					tv4.setText("请再次输入交易密码");
					//Log.i("tradcode",code.toString());
					...
					if(code.toString().equals(code1.toString())){
						String custid=sp.getString("custid", "无");
						if(custid.equals("无")){
							this.finish();
							startActivity(new Intent(Tradecodeactivity.this,Loginactivity.class));
						}else{}
						Md5 md5=new Md5();
						String md5tradecode=md5.stringToMD5(code1.toString());
						if(sp.getString("istradepass", "false").equals("true")){
							urlstring="ResetTradePass/Cust_ID="+custid+"&Cust_TradePass="+md5tradecode+"&Cust_VerifyCode="+intent1.getStringExtra("verifycode");
						}else{
							urlstring="UpdateTradePass/Cust_ID="+custid+"&Cust_TradePass="+md5tradecode;
						}
